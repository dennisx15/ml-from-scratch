"""
Simple Neural Network Framework (from scratch with NumPy)
---------------------------------------------------------
Implements:
...
def softplus(x):
    ...
    # Use approximation for large x to avoid overflow
    return np.where(x > 20, x, np.log1p(np.exp(x)))


def softplus_gradient(x):
    # Gradient of Softplus is the sigmoid function
...
def batch_data(X, y, batch_size, shuffle=False, drop_last=False):
    ...
    Parameters:
        ...
        y (np.ndarray): Labels (one-hot encoded or integers).
...
class OutputLayer:
    def __init__(self, num_inputs, num_outputs, num_layers, activation_function):
        # Initialize input-to-hidden and hidden-to-output weights and biases
        self.input_weights = (2 * np.random.random((num_inputs, num_layers)) - 1) * 0.1
        self.input_biases = (2 * np.random.random((1, num_layers)) - 1) * 0.1
    ...
    def predict(self, x):
        # Forward pass through hidden sub-layer
        ...
        # Final output: logits -> softmax
        logits = np.dot(h1, self.output_weights) + self.output_biases
        return softmax(logits)

    def back_propagate(self, x, y):
        # Forward pass to compute hidden outputs
        ...
        # Backpropagate error to hidden sub-layer
        delta_hidden = np.dot(delta_output, self.output_weights.T) * hidden_grad
...
class NeuralNetwork:
    def __init__(self, hidden_layers, output_layer):
        # Store all hidden layers and output layer
        ...
        for epoch in range(epochs):
            for X_batch, y_batch in batch_data(x, y, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last):
                ...
                for i in reversed(range(len(self.hidden_layers))):
                    ...
                    # Scale gradients by learning rate and batch size
